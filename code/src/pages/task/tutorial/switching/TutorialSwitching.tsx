...
export const TutorialSwitching = () => {
  ...
  const getCurrentText = () => {
    switch (step) {
      ...
      case 2: return <>To select an answer, hold up one hand into a fist with your fingers facing the screen and move it left and right of the camera. <br /> The camera view is broken up into sections by red lines. The stars will change according to the hand's position (represented by the yellow square) in the divided camera view. <br /> Opening your hand to present a flat palm will begin the countdown seen underneath the camera, currently showing "0". When the countdown returns to "0", an answer is submitted. Submissions are turned off for this page. <br /> Try selecting all possible values at least once. Make sure that the whole hand or fist is always visible, otherwise the gesture detection can be unreliable.<br /><br />Once each option has been selected once, the tutorial will move forward.</>
